...
void delay_sys(uint32_t loop){while(loop--);} //5 = 1us
...
if(bit7) TFT_SDA_H; else TFT_SDA_L;TFT_SCL_L;TFT_SCL_H;
if(bit6) TFT_SDA_H; else TFT_SDA_L;TFT_SCL_L;TFT_SCL_H;
if(bit5) TFT_SDA_H; else TFT_SDA_L;TFT_SCL_L;TFT_SCL_H;
if(bit4) TFT_SDA_H; else TFT_SDA_L;TFT_SCL_L;TFT_SCL_H;
if(bit3) TFT_SDA_H; else TFT_SDA_L;TFT_SCL_L;TFT_SCL_H;
if(bit2) TFT_SDA_H; else TFT_SDA_L;TFT_SCL_L;TFT_SCL_H;
if(bit1) TFT_SDA_H; else TFT_SDA_L;TFT_SCL_L;TFT_SCL_H;
if(bit0) TFT_SDA_H; else TFT_SDA_L;TFT_SCL_L;TFT_SCL_H;
TFT_CS_H;
...
void LCD_drawAxis(int x0,int y0,uint8_t *x,uint8_t *y,uint32_t fColor)
{
	uint32_t i;
	uint32_t x_step = (200 - x0) / 5;
	uint32_t y_step = (y0 - 20) / 5;
	
	LCD_drawLine(x0,y0,235,y0,fColor);
	LCD_drawLine(x0,5,x0,y0,fColor);
	
	LCD_drawLine(235,y0,230,y0+5,fColor);
	LCD_drawLine(235,y0,230,y0-5,fColor);
	LCD_drawLine(x0+5,10,x0,5,fColor);
	LCD_drawLine(x0-5,10,x0,5,fColor);
	
	LCD_PutString(192,y0+8,x,fColor,White);
	LCD_PutString(x0+8,15,y,fColor,White);
	LCD_PutString(x0-8+24,y0+8,"0",fColor,White);
	
	for(i = 0; i < 6 ; i++)
	{
		LCD_drawLine(20 + x0+i*x_step, y0,20 + x0+i*x_step, y0-5, fColor);
		LCD_drawLine(x0, y0-i*y_step, x0+5, y0-i*y_step, fColor);
	}
}

void LCD_drawCurve(uint32_t *x,uint32_t *y,uint32_t num)
{
	uint32_t i;
	uint32_t x_max = 0;
	uint32_t x_min = 0x7FFFFFFF;
	uint32_t y_max = 0;
	uint32_t y_min = 0x7FFFFFFF;
	for(i = 0 ; i < num ; i++)
	{
...
//画圆
void draw_circle_8(int xc,int yc,int x,int y,uint32_t fColor)
{
	LCD_drawPoint(xc+x,yc+y,fColor);
	LCD_drawPoint(xc-x,yc+y,fColor);
	LCD_drawPoint(xc+x,yc-y,fColor);
	LCD_drawPoint(xc-x,yc-y,fColor);
	LCD_drawPoint(xc+y,yc+x,fColor);
	LCD_drawPoint(xc-y,yc+x,fColor);
	LCD_drawPoint(xc+y,yc-x,fColor);
	LCD_drawPoint(xc-y,yc-x,fColor);
}
...
void LCD_drawCircle(int xc,int yc,int r,uint32_t fColor)
{
	int x = 0,y = r,d;
	d = 3 - 2 * r;
	...
	while(x <= y)
	{
		draw_circle_8(xc,yc,x,y,fColor);
		if(d < 0)
		{
			d = d + 4 * x + 6;
		}
 ...
 int main(void)
{
	  ...
	  {
			test[i].imag = 0;
			test[i].real= 2*cos(6 * 2 * PI * (float)i / 64) + 1.5*cos(12 * 2 * PI * (float)i / 64) + 1*cos(20 * 2 * PI * (float)i / 64);
		}
		...
		for(i = 0; i < 64 ; i++)
		{
			mod[i] =   sqrt(test[i].real * test[i].real + test[i].imag * test[i].imag);
		}
    ...
    lcd_initial();
		...
		LCD_drawAxis(10,200,"64Hz","100mV",Red);
		
		for(i = 0; i < 64 ; i++)
		{
			LCD_drawLine(20 + i * 3,200,20 + i * 3,200 - mod[i],Red);
		}
		
		
//	  LCD_PutChar(10,250,'0'+ (tmp3 / 100) ,Red,White);
//		LCD_PutChar(18,250,'0'+ (tmp3 % 100 / 10) ,Red,White);
//	  LCD_PutChar(26,250,'0'+ (tmp3 % 10) ,Red,White);
